...
export default function BrandModelGrid({ state, patch, onReady, onClose }) {
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {onClose ? (
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="shrink-0 w-9 h-9 rounded-full border border-slate-200 bg-white text-slate-500 hover:text-slate-900 hover:border-slate-300 font-black"
          >
            ✕
          </button>
        ) : null}
...
function BrandMark({ name, logo }) {
  ...
  const src = logo && !failed ? mediaSrc(logo) : '';
  if (src) {
    return (

    ...
    return (
      <Shell>
        <div className="text-center py-10">
          ...
          <Link to="/dealer/login" className="inline-block mt-6 bg-[#3083ff] text-white font-black rounded-2xl px-6 py-3">
    ...
    <Shell>
      <ol className="grid grid-cols-4 gap-2 mt-8">
        {STEPS.map((item) => {
          ...
          const clickable = !locked && item.id < step;
          return (
            <li key={item.id}>
              <button
                type="button"
                ...
                } ${clickable ? 'cursor-pointer hover:ring-2 hover:ring-[#3083ff]/20' : 'cursor-default'}`}
              >
      ...
      <form
        className="mt-6 rounded-3xl border border-white bg-white/90 p-6 sm:p-8 shadow-sm"
        ...
        {locked && step === 4 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm font-semibold text-slate-500 mt-2 max-w-md mx-auto">
              {approved
                ? 'You can upload cars. 4tyrezz admin reviews each listing before it goes live.'
            ...
            <Link to="/dealer/dashboard" className="inline-block mt-6 bg-[#3083ff] text-white font-black text-xs uppercase tracking-wider rounded-xl px-5 py-3">
              Go to dashboard
            </Link>
          </div>
        ) : (
            ...
            <div className="mt-8 pt-5 border-t border-slate-100">
              ...
              <div className="flex items-center justify-between gap-3">
                <button
                  type="button"
                  onClick={goBack}
                  disabled={step === 1 || loading || locked}
                  className="px-5 py-3 rounded-2xl border border-slate-200 text-sm font-black text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Back
                </button>
                {step < 4 ? (
                  <button
                    ...
                    onClick={goNext}
                    disabled={!canGo}
                    className="px-6 py-3 rounded-2xl bg-[#3083ff] text-white text-sm font-black disabled:opacity-40 disabled:bg-slate-300 disabled:cursor-not-allowed"
                  >
                    {loading ? 'Saving…' : 'Next'}
                  </button>
                ) : (
  ...
  return (
    <div className="bg-slate-50 min-h-[calc(100vh-72px)]">
      <div className="container-px mx-auto px-4 sm:px-6 lg:px-8 py-10 max-w-3xl">
        ...
        <p className="text-sm font-medium text-slate-500 mt-2">Complete each step. Next stays off until the required fields on this page are filled.</p>
        {children}
      </div>
    </div>
  );
